<?php
//http://mode/index.php?callback=monoslideshow&gid=6&preset=melt
...
if ($galleryID == 0) {
	...
	$pictures = $wpdb->get_results("SELECT t.*, tt.* FROM $wpdb->nggallery AS t INNER JOIN $wpdb->nggpictures AS tt ON t.gid = tt.galleryid WHERE t.gid = '$galleryID' AND tt.exclude != 1 ORDER BY tt.sortorder, tt.imagedate");
}
//print_r($pictures);
//print_r($wpdb);
...
echo "\t<configuration onAlbumEnd='$onAlbumEnd' followImageLinks='true' fullScreenScaleMode='$scaleMode' scaleMode='$scaleMode' backgroundColor='$backgroundColor' delay='$delay' randomize='$randomize'>\n";
...
if (is_array ($pictures)){
	foreach ($pictures as $picture) {
        ...
        $link = nggcf_get_field($picture->pid, 'ImageLinksTo');
        ...
        echo "\t\t<image id='$picture->pid' source='$path' link='$link' description='$description' thumbnail='$thumbnail' />\n";             
	}
}
